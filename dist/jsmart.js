...
(function(factory) {
  ...
  function trimAllQuotes(s) {
    return evalString(s.replace(/^['"](.*)['"]$/, "$1")).replace(
      /^\s+|\s+$/g,
      ""
    );
  }
  ...
  var jSmartParser = {
      ...
      for (
        ...
        if (tag) {
          ...
          if (name in this.buildInFunctions) {
            ...
            var params = ("parseParams" in buildIn
              ? buildIn.parseParams.bind(this)
              : this.parseParams.bind(this))(paramStr);
            if (buildIn.type === "block") {
              // Remove new line after block open tag (like in Smarty)
              tpl = tpl.replace(/^\n/, "");
              closeTag = this.findCloseTag("/" + name, name + " +[^}]*", tpl);
              var functionTree = buildIn.parse.call(
                this,
                params,
                tpl.slice(0, closeTag.index)
              );
              if (functionTree) {
            ...
            tree = tree.concat(
              this.parsePluginFunc(name, this.parseParams(paramStr))
            );
            ...
            if (plugin.type === "block") {
              closeTag = this.findCloseTag("/" + name, name + " +[^}]*", tpl);
              tree = tree.concat(
                this.parsePluginBlock(
                  name,
                  this.parseParams(paramStr),
                  tpl.slice(0, closeTag.index)
                )
              );
              tpl = tpl.slice(closeTag.index + closeTag[0].length);
            ...
            // Variable.
            node = this.buildInFunctions.expression.parse.call(
              this,
              openTag[1]
            );
            tree.push(node);
          ...
          if (
            node.expression.type === "build-in" &&
            node.expression.name === "operator" &&
            node.expression.op === "="
          ) {
            tpl = tpl.replace(/^\n/, "");
          }
    ...
    findTag: function(expression, s) {
      ...
      var expressionTag = expression
        ? new RegExp("^\\s*(" + expression + ")\\s*$", "i")
        : expressionAny;
      var sTag;
      var found;
      var literStr = 'literal';
      var signLen = literStr.length + ldelim.length + rdelim.length;
      var getSpace = function(len){
          var str = ''
          for(var j = 0; j < len; j++){
              str += ' ';
          }
      ...
      for (; i < s.length; ++i) {
        if (s.substr(i, ldelim.length) === ldelim) {
          // Ignore literal tags
          if (s.substr(i + ldelim.length).indexOf(literStr + rdelim) == 0) {
            i = s.indexOf(ldelim + '/' + literStr + rdelim);
            if(i >= 0){
    ...
    findElseTag: function(reOpen, reClose, reElse, s) {
      ...
      for (
        var elseTag = this.findTag(reElse, s);
        elseTag;
        elseTag = this.findTag(reElse, s)
      ) {
    ...
    findCloseTag: function(expressionClose, expressionOpen, s) {
      ...
      do {
        ...
        if (!closeTag) {
          throw new Error(
            "Unclosed " + this.ldelim + expressionOpen + this.rdelim
          );
        }
    ...
    lookUp: function(s, value) {
      ...
      if (anyMatchingToken !== false) {
        ...
        var newTree = this.tokens[anyMatchingToken].parse.call(
          this,
          s.slice(RegExp.lastMatch.length),
          { tree: tree, token: RegExp.lastMatch }
        );
    ...
    removeComments: function(tpl) {
      ...
      for (
        var openTag = tpl.match(ldelim);
        openTag;
        openTag = tpl.match(ldelim)
      ) {
    ...
    tokens: [
      ...
      },
      {
        ...
        parse: function(s, data) {
          ...
          var dataMod = this.parseModifiers(
            s.slice(params.toString().length),
            tree
          );
          if (dataMod) {
            return dataMod;
      ...
      },
      {
        ...
        parse: function(s, data) {
          if (
            this.lastTreeInExpression.length &&
            this.lastTreeInExpression[this.lastTreeInExpression.length - 1]
              .type === "var"
          ) {
    ...
    buildInFunctions: {
        ...
        parse: function(s) {
          ...
          return {
            ...
            params: this.parseParams(
              s.slice(data.value.length).replace(/^\s+|\s+$/g, "")
            )
          };
        }
      },
      section: {
        ...
        parse: function(params, content) {
          ...
          if (findElse) {
            ...
            subTreeElse = this.parse(
              content
                .slice(findElse.index + findElse[0].length)
                .replace(/^[\r\n]/, "")
            );
          } else {
            subTree = this.parse(content);
      ...
      config_load: {
        ...
        parse: function(params) {
          ...
          var section = trimAllQuotes(
            params.section ? params.section : params[1] ? params[1] : ""
          );
      ...
      for: {
        ...
        parse: function(params, content) {
          ...
          if (findElse) {
            subTree = this.parse(content.slice(0, findElse.index));
            subTreeElse = this.parse(
              content.slice(findElse.index + findElse[0].length)
            );
          } else {
            subTree = this.parse(content);
          }
      ...
      if: {
        ...
        parse: function(params, content) {
          ...
          if (findElse) {
            ...
            if (findElseIf) {
              subTreeElse = this.buildInFunctions["if"].parse.call(
                this,
                this.parseParams(findElseIf[1]),
                content.replace(/^\n/, "")
              );
            } else {
      ...
      function: {
        ...
        parse: function(params, content) {
          ...
          */
          ...
          this.runTimePlugins[
            trimAllQuotes(params.name ? params.name : params[0])
          ] = true;
      ...
      extends: {
        ...
        parse: function(params) {
          return this.loadTemplate(
            trimAllQuotes(params.file ? params.file : params[0]),
            true
          );
        }
      },
  ...
  var jSmartProcessor = {
      ...
      for (var i = 0; i < tree.length; ++i) {
        ...
        if (node.type === "text") {
          ...
          if (this.runTimePlugins[node.name]) {
            ...
            tmp = this.buildInFunctions["function"].process.call(
              this,
              node,
              data
            );
        ...
        if (data.smarty.continue || data.smarty.break) {
          res = res.replace(new RegExp(data.smarty.ldelim + '(/)?literal' +  data.smarty.rdelim, 'g'), '');
    ...
    getActualParamValues: function(params, data) {
      ...
      actualParams.__get = function(name, defVal, id) {
        ...
        if (
          typeof id !== "undefined" &&
          typeof actualParams[id] !== "undefined"
        ) {
    ...
    getVarValue: function(node, data, value) {
      ...
      for (i = 0; i < node.parts.length; ++i) {
        ...
        if (part.type === "plugin" && part.name === "__func" && part.hasOwner) {
          ...
          if (
            name in data.smarty.section &&
            part.type === "text" &&
            processOutput !== "smarty"
          ) {
    ...
    buildInFunctions: {
        ...
        process: function(node, data) {
          ...
          if (findInArray(params, "nofilter") < 0) {
            ...
            if (this.tplModifiers.length) {
              ...
              res = this.process(
                this.tplModifiers[this.tplModifiers.length - 1],
                data
              );
              if (typeof res !== "undefined") {
                data = res.data;
      ...
      assign: {
        process: function(node, data) {
          ...
          return {
            tpl: "",
            data: this.assignVar(
              params.__get("var", null, 0),
              params.__get("value", null, 1),
              data
            )
          };
        }
      ...
      for: {
        process: function(node, data) {
          ...
          var total = Math.min(
            Math.ceil(
              ((step > 0 ? to - from : from - to) + 1) / Math.abs(step)
            ),
            max
          );

          for (
            var i = parseInt(params.from, 10);
            count < total;
            i += step, ++count
          ) {
            if (data.smarty.break) {
      ...
      block: {
        process: function(node, data) {
          var blockName = trimAllQuotes(
            node.params.name ? node.params.name : node.params[0]
          );
      ...
      call: {
        process: function(node, data) {
          ...
          var s = this.buildInFunctions["function"].process.call(
            this,
            newNode,
            data
          );
      ...
      function: {
        process: function(node, data) {
          ...
          var defaults = this.getActualParamValues(
            funcData.defaultParams,
            data
          );
          ...
          var obj = this.process(
            funcData.tree,
            objectMerge({}, data, defaults, params)
          );
  ...
  */
  ...
  jSmart.prototype = {
    ...
    fetch: function(data) {
      ...
      this.globalAndDefaultModifiers = jSmart.prototype.defaultModifiersGlobal.concat(
        this.defaultModifiers
      );

      // Take default global filters, add with local default filters.
      // Merge them and keep them cached.
      this.globalAndDefaultFilters = jSmart.prototype.filtersGlobal.variable.concat(
      ...
      return this.applyFilters(
        jSmart.prototype.filtersGlobal.post.concat(this.filters.post),
        outputData.output
      );
    },
    ...
    registerFilter: function(type, callback) {
      (this.tree ? this.filters : jSmart.prototype.filtersGlobal)[
        type === "output" ? "post" : type
      ].push(callback);
    },
  ...
  var phpJs = {
    ...
    rawUrlDecode: function(string) {
      return decodeURIComponent(
        (string + "").replace(/%(?![\da-f]{2})/gi, function() {
    ...
    strftime: function(fmt, timestamp) {
      ...
      var str = fmt.replace(/%([aAbBCdegGHIjklmMpPsSuUVwWyYzZ%])/g, function(
        m0,
        m1
      ) {
        var f = _formats[m1];
  ...
  jSmart.prototype.registerPlugin("modifier", "capitalize", function(
    s,
    upDigits,
    lcRest
  ) {
    ...
    var re = new RegExp(
      upDigits ? "[^a-zA-Z_\u00E0-\u00FC]+" : "[^a-zA-Z0-9_\u00E0-\u00FC]"
    );
    var found = null;
    var res = "";
    if (lcRest) {
  ...
  jSmart.prototype.registerPlugin("modifier", "cat", function(s, value) {
    value = value || "";
    return String(s) + value;
  });

  jSmart.prototype.registerPlugin("modifier", "count", function(a) {
  ...
  jSmart.prototype.registerPlugin("modifier", "count_characters", function(
    s,
    includeWhitespaces
  ) {
    s = String(s);
  ...
  jSmart.prototype.registerPlugin("modifier", "count_paragraphs", function(s) {
  ...
  jSmart.prototype.registerPlugin("modifier", "count_sentences", function(s) {
    if (typeof s === "string") {
  ...
  jSmart.prototype.registerPlugin("modifier", "count_words", function(s) {
    if (typeof s === "string") {
  ...
  jSmart.prototype.registerPlugin("modifier", "date_format", function(
    s,
    fmt,
    defaultDate
  ) {
    return phpJs.strftime(
      fmt || "%b %e, %Y",
      phpJs.makeTimeStamp(s || defaultDate)
    );
  });

  jSmart.prototype.registerPlugin("modifier", "debug_print_var", function(s) {
    console.log(s + "--");
  ...
  jSmart.prototype.registerPlugin("modifier", "defaultValue", function(
    s,
    value
  ) {
    value = value || "";
  ...
  jSmart.prototype.registerPlugin("modifier", "escape", function(
    s,
    escType,
    charSet,
    doubleEncode
    ...
    doubleEncode =
      typeof doubleEncode !== "undefined" ? Boolean(doubleEncode) : true;
    var res = "";
    var i;
  ...
  jSmart.prototype.registerPlugin("modifier", "from_charset", function(s) {
    // No implementation in JS. But modifier should not fail hencce this.
    return s;
  });

  jSmart.prototype.registerPlugin("modifier", "indent", function(
    s,
    repeat,
    indentWith
  ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "lower", function(s) {
    return String(s).toLowerCase();
  });

  jSmart.prototype.registerPlugin("modifier", "nl2br", function(s) {
    return String(s).replace(/\n/g, "<br />");
  });

  // only modifiers (flags) 'i' and 'm' are supported
  // backslashes should be escaped e.g. \\s
  jSmart.prototype.registerPlugin("modifier", "regex_replace", function(
    s,
    re,
    replaceWith
  ) {
    ...
    return String(s).replace(
      new RegExp(pattern[1], "g" + (pattern.length > 1 ? pattern[2] : "")),
      replaceWith
    );
  });

  jSmart.prototype.registerPlugin("modifier", "replace", function(
    s,
    search,
    replaceWith
  ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "spacify", function(s, space) {
    if (!space) {
      space = " ";
    }
    return String(s).replace(/(\n|.)(?!$)/g, "$1" + space);
  ...
  jSmart.prototype.registerPlugin("modifier", "string_format", function(
    s,
    fmt
  ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "strip", function(
    s,
    replaceWith
  ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "strip_tags", function(
    s,
    addSpaceOrTagsToExclude,
    tagsToExclude
  ) {
    ...
    if (!tagsToExclude) {
      if (
        addSpaceOrTagsToExclude !== true &&
        addSpaceOrTagsToExclude !== false &&
        (addSpaceOrTagsToExclude + "").length > 0
      ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "to_charset", function(s) {
    // No implementation in JS. But modifier should not fail hencce this.
    return s;
  });

  jSmart.prototype.registerPlugin("modifier", "truncate", function(
    s,
    length,
    etc,
    breakWords,
    ...
    if (middle) {
      ...
      return (
        s.slice(0, Math.floor(length / 2)) +
        etc +
        s.slice(s.length - Math.floor(length / 2))
      );
    }
  ...
  jSmart.prototype.registerPlugin("modifier", "unescape", function(
    s,
    escType,
    charSet
  ) {
  ...
  jSmart.prototype.registerPlugin("modifier", "wordwrap", function(
    s,
    width,
    wrapWith,
    breakWords
    ...
    for (var i = 0; i < lines.length; ++i) {
      ...
      while (line.length > width) {
        ...
        for (
          ;
          found && pos + found.index <= width;
          found = line.slice(pos).match(/\s+/)
        ) {
          pos += found.index + found[0].length;
        }
        pos = breakWords
          ? width - 1
          : pos || (found ? found.index + found[0].length : line.length);
  ...
  jSmart.prototype.registerPlugin("function", "__quoted", function(
    params,
    data
  ) {
    return params.join("");
  ...
  jSmart.prototype.registerPlugin("function", "__array", function(
    params,
    data
  ) {
    var a = [];
  ...
  jSmart.prototype.registerPlugin("function", "__func", function(params, data) {
    var paramData = [];
    var i;
    var fname;
  ...
  jSmart.prototype.registerPlugin("function", "counter", function(
    params,
    data
  ) {
  ...
  jSmart.prototype.registerPlugin("function", "cycle", function(params, data) {
    var name = params.__get("name", "default");
    ...
    if (values) {
      ...
      if (
        arr.length !== data.smarty.cycle[name].arr.length ||
        arr[0] !== data.smarty.cycle[name].arr[0]
      ) {
    ...
    if (
      data.smarty.cycle[name].index >= data.smarty.cycle[name].arr.length ||
      reset
    ) {
      data.smarty.cycle[name].index = 0;
    }
    ...
    if (params.__get("assign", false)) {
      this.assignVar(
        params.assign,
        data.smarty.cycle[name].arr[data.smarty.cycle[name].index],
        data
      );
      return "";
    }
  ...
  jSmart.prototype.registerPlugin("function", "eval", function(params, data) {
    var s = params.var;
    if ("assign" in params) {
  ...
  jSmart.prototype.registerPlugin("function", "debug", function(params, data) {
    ...
    if (env === "browser") {
      ...
      window.jsmartDebug = window.open(
        "",
        "",
      ...
      for (var j in data.includedTemplates) {
        includedTemplates +=
          ...
          (++i % 2 ? "odd" : "even") +
          "><td>" +
          data.includedTemplates[j] +
          "</td></tr>";
      }
      if (includedTemplates !== "") {
        includedTemplates =
          "<h2>included templates</h2><table>" +
          includedTemplates +
          "</table><br>";
      }
      i = 0;
      for (var name in data.assignedVars) {
        assignedVars +=
          ...
          name +
          "]</td><td>" +
          jSmart.prototype.printR(data.assignedVars[name]) +
          "</td></tr>";
      }
      if (assignedVars !== "") {
        assignedVars =
          "<h2>assigned template variables</h2><table>" +
          assignedVars +
          "<table>";
      }
      var html =
      ...
      if (typeof console !== "undefined") {
        console.log(
          "included templates:- " + JSON.stringify(includedTemplates)
        );
        console.log(
          "assigned template variables:- " + JSON.stringify(assignedVars)
        );
      }
    }
    return "";
  ...
  jSmart.prototype.registerPlugin("function", "fetch", function(params, data) {
  ...
  jSmart.prototype.registerPlugin("function", "html_checkboxes", function(
    params,
    data
  ) {
  ...
  jSmart.prototype.registerPlugin("function", "html_image", function(
    params,
    data
  ) {
  ...
  jSmart.prototype.registerPlugin("function", "html_options", function(
    params,
    data
  ) {
    var values = params.__get("values", params.options);
  ...
  jSmart.prototype.registerPlugin("function", "html_radios", function(
    params,
    data
  ) {
    params.type = "radio";
  ...
  jSmart.prototype.registerPlugin("function", "html_select_date", function(
    params,
    data
  ) {
    ...
    if (
      (startYear > endYear && !reverseYears) ||
      (startYear < endYear && reverseYears)
    ) {
      var temp = endYear;
      endYear = startYear;
  ...
  jSmart.prototype.registerPlugin("function", "html_table", function(
    params,
    data
  ) {
    ...
    for (var row = 0; row < rows; ++row) {
      ...
      for (var col = 0; col < cols; ++col) {
        idx =
          inner === "cols"
            ? (vdir === "down" ? row : rows - 1 - row) * cols +
              (hdir === "right" ? col : cols - 1 - col)
            : (hdir === "right" ? col : cols - 1 - col) * rows +
              (vdir === "down" ? row : rows - 1 - row);
        s +=
          "<td" +
          (tdAttr ? " " + tdAttr[col % tdAttr.length] : "") +
          ">" +
          (idx < loop.length ? loop[idx] : trailpad) +
          "</td>\n";
      }
      s += "</tr>\n";
    }
    ...
    if (colNames.length) {
      ...
      for (var i = 0; i < colNames.length; ++i) {
        sHead +=
          "\n<th" +
          (thAttr ? " " + thAttr[i % thAttr.length] : "") +
          ">" +
          colNames[hdir === "right" ? i : colNames.length - 1 - i] +
          "</th>";
      }
      sHead += "\n</tr></thead>";
    }
  ...
  jSmart.prototype.registerPlugin("function", "mailto", function(params, data) {
    var address = params.__get("address", null);
    var encode = params.__get("encode", "none");
    var text = params.__get("text", address);
    var cc = phpJs
      .rawUrlEncode(params.__get("cc", ""))
      .replace(/%40/g, "@")
      .replace(/%2C/g, ",");
    var bcc = phpJs
      .rawUrlEncode(params.__get("bcc", ""))
      .replace(/%40/g, "@")
      .replace(/%2C/g, ",");
    var followupto = phpJs
      .rawUrlEncode(params.__get("followupto", ""))
      .replace(/%40/g, "@")
      .replace(/%2C/g, ",");
    ...
    address += newsgroups
      ? (cc || bcc || subject ? "&" : "?") + "newsgroups=" + newsgroups
      : "";
    address += followupto
      ? (cc || bcc || subject || newsgroups ? "&" : "?") +
        "followupto=" +
        followupto
      : "";
  ...
  jSmart.prototype.registerPlugin("block", "textformat", function(
    params,
    content,
    data,
    repeat
